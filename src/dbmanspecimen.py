                ...
                with self.__connect() as conn:
                    with conn.cursor() as cur:
                        ...
                        if self.chain_table == "chain_moonbeam_moonbase_alpha":
                            cur.execute(
                                r'SELECT * FROM chain_moonbeam_moonbase_alpha."_proof_chain_specimen_events" WHERE observer_chain_session_start_block_id > %s AND observer_chain_finalization_tx_hash IS NULL AND origin_chain_block_height > 18937371;',
                                (self.last_block_id,),
                            )
                        else:
                            cur.execute(
            ...
            while True:
                with self.__connect() as conn:
                    with conn.cursor() as cur:
                        ...
                        if self.chain_table == "chain_moonbeam_moonbase_alpha":
                            cur.execute(
                                r'SELECT * FROM chain_moonbeam_moonbase_alpha."_proof_chain_specimen_events" WHERE observer_chain_session_start_block_id > %s AND origin_chain_block_height > 18937371;',
                                (self.last_block_id,),
                            )
                        else:
                            cur.execute(
